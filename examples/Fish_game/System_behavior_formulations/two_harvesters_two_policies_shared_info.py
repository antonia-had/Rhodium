...
def fish_game(vars, # contains all C, R, W for RBF policy
    ...
    for i in range(N):
        ...
        z_a[0]= hrvSTR([0,0], vars[:10], input_ranges, output_ranges)
        z_b[0]= hrvSTR([0,0], vars[10:], input_ranges, output_ranges)
        ...
        for t in range(tSteps):
            if x[t] > 0 and y[t] > 0:
                ...
                if t <= tSteps-1:
                    z_a[t+1]= hrvSTR([z_a[t]*x[t],z_b[t]*y[t]], vars[:10], input_ranges, output_ranges)
                    z_b[t+1]= hrvSTR([z_a[t]*x[t],z_b[t]*y[t]], vars[10:], input_ranges, output_ranges)
            prey[i,t+1] = x[t+1]
