...
for l in range(ncols*nrows):
    ax = fig.add_subplot(nrows,ncols,l+1)
    ax.fill_between(range(len(sharedinfo_robustness[:,0])),np.sort(sharedinfo_robustness[:,l])[::-1],color='#08519c')
    ax.fill_between(range(len(noinfo_robustness[:,0])),np.sort(noinfo_robustness[:,l])[::-1],color='#a50f15')
    ax.set_ylim([0,100])
    if l==0 or l==4:
        ax.set_ylabel('Percent of Sampled SOWs in which criterion is met')
    ax.set_xlabel('Solution # (sorted by rank)')
    ax.tick_params(axis='both')
    ax.set_title(criteria[l])
...
for l in range(ncols*nrows):
    ...
    ax.tick_params(axis='both')
    ax.set_title(criteria[l])
plt.savefig('robustness_comparison_reverse.png')
plt.close()
...
#ITpolicySOWs = load("reevaluation_"+str(ITpolicy)+".csv")[1]
#
